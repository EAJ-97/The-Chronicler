...
function buildExportPayload(db, rootFolderId) {
  ...
  const connections = db
    .prepare(
      `SELECT id, source_note_id, target_note_id, label, is_speculative, created_by, created_at
...
function runImport(db, data, opts) {
  ...
  const run = () => {
    ...
    const insConn = db.prepare(
      `INSERT INTO connections (source_note_id, target_note_id, label, is_speculative, created_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    );
    for (const c of data.connections || []) {
      ...
      if (s && t && by) {
        try {
          insConn.run(s, t, c.label ?? '', c.is_speculative ?? 0, by, c.created_at);
        } catch (e) {
